...
Please download the data used in the course \href{https://www.kaggle.com/datasets/thedevastator/exploring-nba-player-performance-and-salaries-19?select=salaries_1985to2018.csv}{here}. To find more about me, have a look at \href{https://jaspertjaden.com}{my website}.
...
R Markdown is a file format that combines R code and plain text. The file extension is \texttt{.Rmd}. You can write R Markdown files in any text editor, but it is recommended to use an IDE (integrated development environment) like RStudio, which provides many features to help you write and preview your documents.

R Markdown files consist of three main parts:

\begin{itemize}
\item
  A YAML header that contains metadata and options for your document, such as title, author, date, output format, etc. The header starts and ends with three dashes (\texttt{-\/-\/-}).
\item
  A body that contains the main content of your document, written in plain text with Markdown syntax. Markdown is a simple way to format your text using symbols like \texttt{\#} for headings, \texttt{*} for emphasis, \texttt{{[}link{]}(url)} for hyperlinks, etc. You can also embed R code chunks in your document using three backticks (\texttt{markdown}) followed by \texttt{\{r\}} and some optional chunk options. The code chunks will be executed when you knit your document and the results will be displayed in the output.
\item
  A final code chunk that contains any cleanup or summary code that you want to run at the end of your document. This chunk is optional and starts with three backticks followed by \texttt{\{r,\ echo\ =\ FALSE\}}.
\end{itemize}
