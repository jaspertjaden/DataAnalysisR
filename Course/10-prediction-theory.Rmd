...
All what we have done so far can be considered as part of classical statistical
...
how the outcome can be explained but to predict something with it which we want
to know. 
Making predictions based on a computed model is hardly a new approach, still it
...
with as little error as possible. The line will have an an intercept, i.e. where
the line crosses the Y-Axis, $b_0$ in our formula, and a slope, $b_1$.
...
For every $x$ value on our axis, the formula describes the corresponding $y$
value. We can for example compute the $y$ values our model assumes for $x = 10$
and $x = 20$.
...
function does is computing a linear regression for the two variables in the plot
and drawing the corresponding regression line.
...
The two marked points are the values for $y$ our model estimates for $x = 10$
...
are in the data used to compute the model. The model simply does not know what
...
stopping us from doing so. In fact we have done so [before0](#lin-a). 
...
best fir. They are saved as a variable in the model object and can be extracted
...
and have `augment()` add the values from the model to the original data.
...
the goal was just that, estimatiom of an effect. Now the goal has changed. We
...
season an observation, and thus their salary, was recorded. We also added an age
variable to the DAG. This was not present before, but should also have an impact
on the salary as it represents the time a player has already spent playing and 
thus their experience.

We can now run a new model including these additional variables. We also 
construct a new objext holding the original data and the predictions made by
`m2`.
...
There are mutltiple approaches to comparing the accuracy of two linear regression
...
in the outcome can be explained by your set of independent variables.
...
about $42.8\%$, an increase of $3\%$. Not much but the $R^2$ indicates that `m2`
will be able to make somewhat better predictions compared to `m2`.
...
values is in computing the mean. Without going into the reasons, this makes
...
MSE but it presents us a measure that is interpretable in the units of the
outcome variable.
...
but if we compare it ti the overall RMSE of model 2, our predictions are still
substantially off.

We could try to improve our model by thinking about further variables to include
...
played on make for the salary received. We can approach this by using 
...
would include the true population value." This leaves $5\%$ of repetitions
where the confidecne intervals does not include the true value. If we are in the
$95\%$ or the $5\%$ is unknown to us. But what we can say, is that we are $95\%$
confident that our confidence interval includes the true value.
In practice the confidence interval tells us the range of values for which we
have this confidence of $95\%$, without knowing if it is actually true in our 
case.
...
the true value is included differs by observation. For example, instead of
stating that a center who scores $10$ points per game, is 20 years
of age and plays for the Knicks in 2017 will most definitely make $4,474,038\$$,
we would state that we are $95\%$ confident that the true population value for
such a player lies between $3,920,882\$$ and $5,105,234\$$.
...
[here](https://vitalflux.com/mean-square-error-r-squared-which-one-to-use/)
