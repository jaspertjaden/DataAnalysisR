...
We can use `read_delim()` or `read_csv` function. Note that you need other function for Stata datasets (`read_data` from teh haven package). To load Rdata files, you use the `load()` function.
...
```{r import_csv}
# import data 
nba_salaries <- read_csv("../datasets/nba/salaries_1985to2018.csv")
nba_players <- read_csv("../datasets/nba/players.csv")
```
...
```{r merge}
data_nba <- merge(nba_players, nba_salaries, by.x = c("id"), by.y=c("player_id"))
...
There are a number of cool things that regressioncan do for us that simple EDA cannot: 
