...
::: {.functions}
- `load()`: loads an R data file into the current session.
- `read_excel()`: reads an Excel file into a data frame.
- `str()`: displays the structure of an object.
- `glimpse()`: provides a transposed version of `str()`.
- `table()`: creates a contingency table of counts for categorical variables.
- `summary()`: produces a summary of an object, such as its mean, median, quartiles, etc.
- `mutate()`: adds new variables or modifies existing ones in a data frame.
- `case_when()`: creates a new variable based on multiple conditions.
- `ggplot()`: creates a plot using the grammar of graphics.
- `corr()`: computes the correlation matrix of numeric variables.
- `skim()`: provides a summary of the whole dataset with various statistics and types of variables.
- `introduce()`: gives an overview of the types and missingness of variables in a data frame.
- `plot_intro()`: plots the information from `introduce()` as a bar chart.
- `plot_missing()`: plots the percentage of missing values for each variable in a data frame.
- `plot_bar()`: plots the frequency distribution of categorical variables in a data frame.
- `tbl_summary()`: creates a summary table for descriptive statistics of variables in a data frame.
- `correlate()`: creates a correlation data frame from a numeric data frame.
- `rearrange()`: rearranges the correlation data frame by the absolute value of correlations.
- `shave()`: removes the upper triangle of the correlation matrix to avoid redundancy.
- `fashion()`: formats the correlation data frame with rounded values and signs.
- `rplot()`: plots the correlation matrix as a scatterplot matrix with regression lines.
- `ggcorrplot()`: plots the correlation matrix as a heatmap with circles representing the magnitude of correlations.
...
2. Install R and Rstudio. If you don't already have R installed, here is a [link](https://rstudio-education.github.io/hopr/starting.html#rstudio) to how it is done.
